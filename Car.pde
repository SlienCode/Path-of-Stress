...
class Car extends Object {
  ...
  Car(int x,int y) {
    ...
    for (int i = 0; i < leftsize; i++) {
      if (i<=130) {
        hitboxleft[i] = new Point(x+48, y-i+256);
      } else if (i<=154) {
        hitboxleft[i] = new Point(x+88, y-i+256);
      } else if (i<=178) {
        hitboxleft[i] = new Point(x+126, y-i+256);
      } else {
        hitboxleft[i] = new Point(x+164, y-i+256);
      }
    }
    
    for (int i = 0; i < upsize; i++) {
      if (i<=40) {
        hitboxup[i] = new Point(x+i+48, y+126);
      } else if (i<=78) {
        hitboxup[i] = new Point(x+i+48, y+102);
      } else if (i<=115) {
        hitboxup[i] = new Point(x+i+48, y+78);
      } else if (i<=286) {
        hitboxup[i] = new Point(x+i+48, y+56);
      } else {
        hitboxup[i] = new Point(x+i+48, y+118);
      }
    }
    ...
    for (int i = 0; i < rightsize; i++) {
      if (i<=138) {
        ...
        hitboxright[i] = new Point(x+334, y-i+256);
      }
    }
  }
